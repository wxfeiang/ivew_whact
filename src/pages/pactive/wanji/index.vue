...
import * as wjUtils from '../../../../static/js/wj/wjUtils'
        ...
        if (serviceResult.serviceCode === 0) {
          ...
          wjBleAPI.getObuSysInfo(function (serviceResult) {
            if (serviceResult.serviceCode === 0) {
              ...
              console.log('readObuSysInfo success: ' + str)
              that.gotoHome()
              // that.setData({ msg: str })
            } else {
              console.log('readObuSysInfo serviceCode fail')
              // that.setData({ msg: serviceResult.serviceInfo })
            }
          })
        } else {
          // that.setData({ msg: serviceResult.serviceInfo })
          console.log('readObuSysInfo fail')
        }
      })
    },
    convertLisenceNo (e) {
